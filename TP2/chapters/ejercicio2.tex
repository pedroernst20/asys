...
\textbf{A)} Expresar la Ecuación Diferencial (ED) que describe el circuito, y con ello, definir un sistema en tiempo continuo con entrada x(t) = v(t) y salida y(t) = i(t) , con condiciones iniciales genérica.
...
Ahora derivamos para conseguir la expresi\'on buscada

$$\dfrac{d v(t)}{dt} = R \dfrac{d i(t)}{dt} + \dfrac{1}{C} \cdot i(t)$$

Dividimos todo por $R$ para normalizar la ecuaci\'on
...
Primero sintetizamos la ecuaci\'on correspondiente al circuito de primer orden
...
Ya nos queda sintetizado a la expresi\'on
...
Quedando la ecuaci\'on sintetizada
...
El diagrama de bloques que describe a la ecuaci\'on de primer orden es:
...
Por otro lado el que describe a la ecuaci\'on de segundo orden
...
El polo con estos datos ser\'ia $s=-\dfrac{2}{3}$
...
Para la ecuaci\'on de segundo orden 

$$H(s) = \dfrac{\dfrac{s}{L}}{s^2 + \dfrac{R}{L} s + \dfrac{1}{LC}}$$

Y los polos se encontrar\'ian en $$s_n=\dfrac{-\dfrac{R}{L} \pm \sqrt{\dfrac{R}{L}^2-\dfrac{4}{LC}}}{2}$$

Donde reemplazando con los valores previamente dados, los polos quedar\'ian en $(-1,0)$ y $(-2,0)$.
...
Continuaremos resolviendo primero la ecuaci\'on de primer orden
...
Para la ecuaci\'on de primer orden
...
En el caso de la de segundo orden
...
En ambos casos vamos a usar que $\dfrac{d}{dt} [f(t)u(t)] = f(0)\delta (t) + f'(t) u(t)$, siendo para la ecuaci\'on de primer orden
...
Por lo cual se verifica para la primera ecuaci\'on, por otro lado para la de segundo grado
...
Primero transformamos la integral del escal\'on
...
Entonces para calcular la respuesta a la rampa con la ecuaci\'on de primer orden
...
Para el caso de la ecuaci\'on de segundo orden, recordamos que
...
Vamos a completar este ejercicio realizando al verificaci\'on de los c\'alculos mediante la siguiente igualdad $y(t) = \int_{-\infty}^{t} v(\tau) d\tau$.

Manteniendo el orden de realizaci\'on de ejercicios, comenzamos con la ecuaci\'on de primer orden
...
Verificando correctamente para la primera ecuaci\'on, ahora correspondemos con la de segundo orden
